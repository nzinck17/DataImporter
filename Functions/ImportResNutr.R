...
# Read Tables
# WQ
query.wq <- dbGetQuery(con, "SELECT max(ID) FROM tbl_Nutrients")
# Get current max ID
if(is.na(query.wq)) {
  query.wq <- 0
} else {
...
col.order.wq <- dbListFields(con, "tbl_Nutrients")
...
IMPORT_DATA <- function(df.wq, path, file, filename.db, processedfolder, df.flags = NULL){
  ...
  # WQ Data
  ColumnsOfTable <- sqlColumns(con, "tbl_Nutrients")
  varTypes  <- as.character(ColumnsOfTable$TYPE_NAME) 
  sqlSave(con, df.wq, tablename = "tbl_Nutrients", append = T, 
