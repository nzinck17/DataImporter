...
# datatype <- "Turbidity" # Options are Tributary or Reservoir, Forestry (other?)
# 
...
# }
#  filename.db <- "DBQ=C:/WQDatabase/QUABBIN_WQDB_fe.mdb" #### TEMPORARY _ DELETE LATER
# ## Add paths for other data types as necessary
# 
# ### Find and set the file to import ###
# datatype <- "Turbidity"
# rawdatafolder <- "W:/WatershedJAH/EQStaff/WQDatabase/OtherData"
# processedfolder <- "W:/WatershedJAH/EQStaff/WQDatabase/OtherData"
# filename.db <- "DBQ=C:/WQDatabase/WaterQualityDB_fe.mdb" # This needs to point to the front end database
# 
# files <- grep(
...
update <- as.numeric(df.wq$ResultReported, digits = 6)
df.wq$ResultReported <- as.character(update)
...
# 
# if (datatype == "Preliminary Bacteria") {
#   df.flags.prelim <- df.wq[,1]
#   names(df.flags.prelim) = "SampleFlag_ID"
#   df.flags.prelim$FlagCode <- 102
#   df.flags.prelim$ID <- seq.int(nrow(df.flags.prelim)) + max(df.flags$ID)
#   df.flags <- bind_rows(df.flags,df.flags.prelim)
#   rm(df.flags.prelim)
# } else {
#   df.flags <- df.flags
# }    
...
IMPORT_DATA <- function(df.wq, df.flags, path, file, filename.db, processedfolder){

  ### First we need to clear out any preliminary data   
  # Connect to db using ODBC
  # filename.db <- "DBQ=C:/WQDatabase/WaterQualityDB_fe.mdb" # This needs to point to the front end database
  # con <- dbConnect(odbc::odbc(),
  #                  .connection_string = paste("driver={Microsoft Access Driver (*.mdb, *.accdb)}",
  #                                             filename.db, "Uid=Admin;Pwd=;", sep = ";"),
  #                  timezone = "America/New_York")
  # 
  # # First check if there is any preliminary data and flags to delete that are contained in the import dataset
  ...
  # qryDelete <- dbGetQuery(con,qryS)
  # # Delete preliminary data (IDs flagged 102 in period of question)
  ...
  # print(paste(dbGetRowsAffected(rs), "preliminary records were deleted during this import", sep = " "))
  # dbClearResult(rs)
  # 
  # # Delete flags associated with preliminary data - Will also delete any other flag associated with record number
