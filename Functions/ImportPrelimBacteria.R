...
# datatype <- "Preliminary Bacteria"
# rawdatafolder <- "W:/WatershedJAH/EQStaff/WQDatabase/MWRA_Data"
# processedfolder <- "W:/WatershedJAH/EQStaff/WQDatabase/MWRA_Data"
# 
# files <- grep(
...
df.wq <- read_excel(path, sheet= 3, col_names = T, trim_ws = T, na = "nil") %>% 
...
if (!anyNA(df.wq$FlagCode) = FALSE){ # Condition returns FALSE if there is at least 1 non-NA value, if so proceed
# Flags
...
# Make another set of flags for every record in data since this is all preliminary data
  df.flags.prelim <- df.wq[,1] # Grab the ID column first 
  names(df.flags.prelim) = "SampleFlag_ID" # Change name of Column
  df.flags.prelim$FlagCode <- 102 # Make the flag Code for preliminary Data
  df.flags.prelim$ID <- seq.int(nrow(df.flags.prelim)) + max(df.flags$ID) # Assign new IDs starting after the last Id of other flags
  df.flags <- bind_rows(df.flags,df.flags.prelim) # Bind the flags into 1 df
  rm(df.flags.prelim) # Remove the temporary flag df
...
# This is preliminary data, so it shouldn't be in the database yet
  ...
  # No need to move any files since the file with data compiles multiple prelim datasets and moves them prior after pre-processing
}
### END 
